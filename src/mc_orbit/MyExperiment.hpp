...
class MyExperiment {
    ...
    shared_ptr<EpisodeRecord> m_episodeRecord;

public:
    MyExperiment(ExperimentConfig config, ControllerConfig ctrlConfig, int trialIndex, int rngSeed, bool learn)
        : m_config(config)
        ...
        , m_speedManager((int) config.renderSteps, config.simTimeStep)
    {
        ...
        if (learn)
            m_episodeRecord = make_shared<EpisodeRecord>(m_world->width(), m_world->height());
    }

    /**
     * Should be executed at the end of the experiment.
     */
    shared_ptr<EpisodeRecord> getEpisodeRecord()
    {
        return m_episodeRecord;
    }
    ...
    void run()
    {
        bool running = true;
        while (running) {
            m_eval = MCOEval::PuckGridValues(m_world, "red_puck", 2, false);
...
/*
double BEFORE = MCOEval::NanPucks(m_world, "red_puck");
...
/*
double AFTER = MCOEval::NanPucks(m_world, "red_puck");
if (BEFORE == 0 && AFTER > 0) {
cerr << "NAN" << endl;
for (auto e1 : m_world->getEntities("red_puck"))
{
...
*/
            ...
            if (m_episodeRecord != nullptr)
                m_episodeRecord->add(m_world, "red_puck", 2);
...
//auto controller = dynamic_pointer_cast<EntityController_MCO>(base);
...
private:
    ...
    {
        /*
        double normPuckRadius = MCOEval::BiggestGridValueDifference(m_world, 0, (int)m_config.puckRadius);
        double normRobotRadius = MCOEval::BiggestGridValueDifference(m_world, 0, (int)m_config.robotRadius);
        ...
        */
        ...
        for (auto e : m_world->getEntities("robot")) {
            e.addComponent<CController>(make_shared<EntityController_MCO>(e, m_world, m_ctrlConfig));
        }
    }
    ...
    void resetSimulator()
    {
        ...
        if (m_world == NULL)
            m_world = mc_orbit_world::GetWorld(m_rng, m_config);
