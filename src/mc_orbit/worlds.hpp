...
Entity addRobot(std::shared_ptr<World> world, ExperimentConfig config)
{
    ...
    // Positioon of plow tip (f for forward sensor).
    // FOR GATHERING:
    ...
    // FOR DISPERSION:
    //double fx = config.plowLength * cos(plowAngleRad) - config.puckRadius;
    //double fy = config.plowLength * sin(plowAngleRad) - 2*config.puckRadius;
    ...
    //sensors.gridSensors.push_back(make_shared<GridSensor>(robot, "gridForward0", 0, config.plowAngleDeg, config.plowLength));
