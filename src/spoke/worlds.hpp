...
return true;
    ...
    void addRobot(std::string team, std::shared_ptr<World> world, size_t x, size_t y, ExperimentConfig config)
    {
        ...
        if (!config.fakeRobots) {
            robot.addComponent<CCircleBody>(innerShellRadius, true);
            robot.addComponent<CCircleShape>(innerShellRadius);
            robot.addComponent<CColor>(red, green, blue, 255);
        }

        // Add the spoke.
        robot.addComponent<CSpoke>(-0.8, innerShellRadius, -0.2, robotRadius);

        // Add the outer shell.
        robot.addComponent<COuterShell>(robotRadius);
